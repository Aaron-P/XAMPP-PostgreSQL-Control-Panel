...
uses
  ...
  uApache, uMySQL, uPostgreSQL, uFileZilla, uMercury, uNetstat, uNetstatTable,
  Menus, IniFiles, uProcesses, AppEvnts, ImgList, JCLDebug, JCLSysInfo;
...
type
  TfMain = class(TForm)
    ...
    bMySQLAdmin: TBitBtn;
    bPostgreSQLAction: TBitBtn;
    bPostgreSQLAdmin: TBitBtn;
    ...
    bMySQLLogs: TBitBtn;
    bPostgreSQLConfig: TBitBtn;
    bPostgreSQLLogs: TBitBtn;
    ...
    PostgreSQLTrayControl: TMenuItem;
    ...
    procedure bPostgreSQLActionClick(Sender: TObject);
    procedure bPostgreSQLAdminClick(Sender: TObject);
    procedure bPostgreSQLConfigClick(Sender: TObject);
    procedure bPostgreSQLLogsClick(Sender: TObject);
    ...
    procedure bPostgreSQLServiceClick(Sender: TObject);
    ...
    procedure PostgreSQLTrayControlClick(Sender: TObject);
...
procedure TfMain.bPostgreSQLActionClick(Sender: TObject);
begin
  if PostgreSQL.isRunning then
    PostgreSQL.Stop
  else
    PostgreSQL.Start;
end;

procedure TfMain.PostgreSQLTrayControlClick(Sender: TObject);
begin
  if PostgreSQL.isRunning then
    PostgreSQL.Stop
  else
...
procedure TfMain.bPostgreSQLAdminClick(Sender: TObject);
begin
  PostgreSQL.Admin;
end;
...
procedure TfMain.bPostgreSQLServiceClick(Sender: TObject);
var
  oldIsService: Boolean;
begin
  oldIsService := PostgreSQL.isService;
  ...
  if PostgreSQL.isService then
  begin
    if MessageDlg(Format(_('Click Yes to uninstall the %s service'), [PostgreSQL.ModuleName]), mtConfirmation, [mbYes, mbNo], 0) = mrYes then
    begin
      updateTimerStatus(False);
      PostgreSQL.ServiceUnInstall;
      updateTimerStatus(True);
    end
    else
      exit;
  ...
  begin
    if MessageDlg(Format(_('Click Yes to install the %s service'), [PostgreSQL.ModuleName]), mtConfirmation, [mbYes, mbNo], 0) = mrYes then
    begin
      updateTimerStatus(False);
      PostgreSQL.ServiceInstall;
      updateTimerStatus(True);
    end
    else
      exit;
  end;
  PostgreSQL.CheckIsService;
  if (oldIsService = PostgreSQL.isService) then
  begin
    PostgreSQL.AddLog(_('Service was NOT (un)installed!'), ltError);
    if (WinVersion.Major = 5) then // WinXP
      PostgreSQL.AddLog
        (_('One possible reason for failure: On windows security box you !!!MUST UNCHECK!!! the "Protect my computer and data from unauthorized program activity" checkbox!!!'),
        ltError);
  ...
  begin
    PostgreSQL.AddLog(_('Successful!'));
  end;
end;
...
//    else
  ...
  if Config.EnableModules.PostgreSQL then
  begin
    PostgreSQL := tPostgreSQL.Create(bPostgreSQLService, pPostgreSQLStatus, lPostgreSQLPIDs, lPostgreSQLPorts, bPostgreSQLAction, bPostgreSQLAdmin);
  end
  else
  begin
    AddLog(_('The PostgreSQL module is disabled'), ltInfo);
    fmain.PostgreSQLTray.Enabled := false;
    fmain.bPostgreSQLService.Enabled := false;
    fmain.bPostgreSQLAction.Enabled := false;
    fmain.bPostgreSQLAdmin.Enabled := false;
    fmain.bPostgreSQLConfig.Enabled := false;
    fmain.bPostgreSQLLogs.Enabled := false;
  end;
  ...
  if Config.EnableModules.PostgreSQL then
  begin
    if Config.ASPostgreSQL then
    begin
      PostgreSQL.AutoStart := True;
      AddLog(Format(_('Enabling autostart for module "%s"'), [PostgreSQL.ModuleName]));
    end;
  end;
  if Config.EnableModules.FileZilla then
  begin
