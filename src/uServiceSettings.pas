...
type
  TfServiceSettings = class(TForm)
    ...
    tApacheSSL: TEdit;
    gMySQL: TGroupBox;
    gPostgreSQL: TGroupBox;
    lMySQLMain: TLabel;
    lMySQLName: TLabel;
    tMySQLMain: TEdit;
    tMySQLName: TEdit;
    lPostgreSQLMain: TLabel;
    lPostgreSQLName: TLabel;
    tPostgreSQLMain: TEdit;
    tPostgreSQLName: TEdit;
...
begin
  ...
  if ValidateName(tPostgreSQLName.Text) then
    Config.ServiceNames.PostgreSQL := tPostgreSQLName.Text
  else
  begin
    ShowMessage('PostgreSQL Name: "' + tPostgreSQLName.Text + '" is not a valid name.');
    error := True;
  end;
  if ValidatePort(tPostgreSQLMain.Text) then
    Config.ServicePorts.PostgreSQL := StrToInt(tPostgreSQLMain.Text)
  else
  begin
    ShowMessage('PostgreSQL: "' + tPostgreSQLMain.Text + '" is not a valid number.');
    error := True;
  end;
...
begin
  ...
  tPostgreSQLName.Text := Config.ServiceNames.PostgreSQL;
  tPostgreSQLMain.Text := IntToStr(Config.ServicePorts.PostgreSQL);
